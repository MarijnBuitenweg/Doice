...
const STATNAMES: [&'static str; 6] = ["str", "dex", "con", "int", "wis", "cha"];
const SKILLNAMES: [&'static str; 18] = ["acr", "anh", "arc", "ath", "dec", "his", "ins", "int", "inv", "med", "nat", "pct", "pfm", "pss", "rel", "soh", "ste", "sur"];
//const CLASSNAMES: [&'static str; 13] = dnd_character::CLASSNAMES.iter().map(|s| s.to_lowercase().as_str() ).collect();
const CATNAMES: [&'static str; 6] = ["general", "stats", "saves", "skills", "cantrips", "spells"];
...
fn interpret_general(lines: Vec<(&str, &str)>) -> Result<[usize; GENERALNAMES.len()], &'static str> {
    let mut out: [usize; GENERALNAMES.len()] = [0; GENERALNAMES.len()];
